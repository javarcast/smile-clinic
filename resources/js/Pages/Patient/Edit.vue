...
onMounted(() => {
  ...
  props.medicament_patient.forEach(element => {
    form.medicaments.push(element.medicament_id)
  });
  props.disease_patient.forEach(element => {
    form.diseases.push(element.disease_id);
  });
});
        ...
        <form @submit.prevent="submit" class="create-user">
              ...
              <table class="w-full">
                ...
                <tbody>
                  <tr v-for="disease in form.diseases" :key="disease">
                    ...
                    <td class="flex justify-center items-center"><button @click.prevent="removeDisease(disease)"><i
                          class="fas fa-trash-alt text-red-600"></i></button></td>
                  </tr>
                  <tr v-for="medicament in form.medicaments" :key="medicament">
                    <td>{{ getMedicamentName(medicament) }}</td>
                    <td>Medicamento</td>
                    <td class="flex justify-center items-center"><button
                        @click.prevent="removeMedicament(medicament)"><i
                          class="fas fa-trash-alt text-red-600"></i></button></td>
                  </tr>
                </tbody>
              </table>
          ...
          <div class="w-1/3 flex flex-col items-center">
            <div class="p-2 rounded bg-gray-200" v-show="getDiseases.length > 0">
              ...
              <div class="flex">
                <select v-model="diseaseItem" class="text-xs w-64 rounded border border-gray-300" name="" id="">
                  <option value="">Selecciona una enfermedad</option>
                  <option v-for="disease in getDiseases" :key="disease.id" :value="disease.id">
                    {{ disease.name }}
                  </option>
                </select>
                <button @click.prevent="addDisease()" class="ml-2 text-white bg-black h-8 w-8 rounded-full">
            ...
            <div class="p-2 rounded bg-gray-200" v-show="getAllergies.length > 0">
              ...
              <div class="flex">
                <select v-model="allergyItem" class="text-xs w-64 rounded border border-gray-300" name="" id="">
                  <option value="">Selecciona una alergia</option>
                  <option v-for="allergy in getAllergies" :key="allergy.id" :value="allergy.id">
                    {{ allergy.name }}
                  </option>
                </select>
                <button @click.prevent="addAllergy()" class="ml-2 text-white bg-black h-8 w-8 rounded-full">
                  <i class="fas fa-plus"></i>
                </button>
              </div>
            </div>
            ...
            <div class="p-2 rounded bg-gray-200" v-show="getMedicaments.length > 0">
              ...
              <div class="flex">
                <select v-model="medicamentItem" class="text-xs w-64 rounded border border-gray-300" name="" id="">
                  <option value="">Selecciona un medicamento</option>
                  <option v-for="medicament in getMedicaments" :key="medicament.id" :value="medicament.id">
                    {{ medicament.name }}
                  </option>
                </select>
                <button @click.prevent="addMedicament()" class="ml-2 text-white bg-black h-8 w-8 rounded-full">
                  <i class="fas fa-plus"></i>
                </button>
              </div>
            </div>
