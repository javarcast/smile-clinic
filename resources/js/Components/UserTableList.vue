<template>
  <table class="bg-white shadow-lg rounded-sm border border-slate-200 relative">
    <thead class="text-xs font-semibold uppercase text-slate-500 bg-slate-50 border-t border-b border-slate-200">
        <tr >
            <th class="px-2 first:pl-5 last:pr-5 py-4 text-left font-semibold">DNI</th>
            <th class="px-2 first:pl-5 last:pr-5 py-4 text-left font-semibold" >Nombre</th>
            <th class="px-2 first:pl-5 last:pr-5 py-4 text-left font-semibold">Email</th>
            <th class="px-2 first:pl-5 last:pr-5 py-4 text-left font-semibold">Nro. Telefonico</th>
            <th class="px-2 first:pl-5 last:pr-5 py-4 text-center font-semibold" colspan="3"> Acciones</th>
        </tr>
    </thead>
    <tbody>

    <tr v-for="user in props.users.data" :key="user.id" class="border-t border-b border-slate-200">
      <td class="px-2 first:pl-5 last:pr-5 py-4 text-left">
        {{ user.id }}
      </td>
      <td class="px-2 first:pl-5 last:pr-5 py-4 text-left">
        {{ user.name }}
